...
class Ref_Province_Model extends MY_Model
{
	/**
	...
	protected $_table = 'tbrProvince';
	protected $primary_key = 'ProvinceCode';
    ...
    //
    //
	protected $_id 			 =  'ProvinceCode';
	protected $_name 		 =  'ProvinceName';
	...
	public function check_blood($name = '')
	{
		...
		return $this->db->where('ProvinceName', $name)
						->count_all_results($this->_table) > 0;
	}

	public function delete_province($id)
	{
		$delete = $this->db->delete($this->_table, array('ProvinceCode' => $id));
		if ($delete)
            return true;
        else
            return false;
	...
	public function add_province($name, array $data)
	{
	...
	public function change_province($id, array $data)
	{
		$result = $this->db->where('ProvinceCode', $id)
			->count_all_results($this->_table) > 0;
		...
		$this->db->update($this->_table, $data, array('ProvinceCode' => $id));
	...
	public function record()
	{
	    ...
	    if ($record->num_rows() > 0) {
            ...
            foreach ($record->result() as $row) {
                ...
                $bool_change = $this->auth->_allowed('Dashboard.Reference.Province.Change');
                $bool_delete = $this->auth->_allowed('Dashboard.Reference.Province.Delete');                
                ...
                if ($bool_change || $bool_delete)
                {
                    ...
                    if ($bool_change)
                        $text .= "<a class='btn btn-small btn-success' onclick='editB(\"".$row->ProvinceCode."\", \"".$row->ProvinceName."\")'>".lang('edit')."</a>";
                    if ($bool_delete)
                        $text .= "<a class='btn btn-small btn-success' onclick='deleteB(\"".$row->ProvinceCode."\", \"".$row->ProvinceName."\")'>".lang('hapus')."</a>";
                    $text .= "</div>";
                }
