...
class Ref_Selection_Steps_Model extends MY_Model
{
	/**
	...
	protected $_table = 'tbrSelectionSteps';
	protected $primary_key = 'IDSteps';
    ...
    //
    //
	protected $_id 			 =  'IDSteps';
	protected $_name 		 =  'Steps';
	...
	public function check($name = '')
	{
		...
		return $this->db->where('Steps', $name)
						->count_all_results($this->_table) > 0;
	}

	public function delete($id)
	{
		$delete = $this->db->delete($this->_table, array('IDSteps' => $id));
		if ($delete)
            return true;
        else
            return false;
	...
	{
		$result = $this->db->where('IDSteps', $id)
			->count_all_results($this->_table) > 0;
		...
		$this->db->update($this->_table, $data, array('IDSteps' => $id));
	...
	public function record()
	{
	    ...
	    if ($record->num_rows() > 0) {
            ...
            foreach ($record->result() as $row) {
                ...
                $bool_change = $this->auth->_allowed('Dashboard.Reference.SelectionSteps.Change');
                $bool_delete = $this->auth->_allowed('Dashboard.Reference.SelectionSteps.Delete');                
                ...
                if ($bool_change || $bool_delete)
                {
                    ...
                    if ($bool_change)
                        $text .= "<a class='btn btn-small btn-success' onclick='editB(\"".$row->IDSteps."\", \"".$row->Steps."\")'>".lang('edit')."</a>";
                    if ($bool_delete)
                        $text .= "<a class='btn btn-small btn-success' onclick='deleteB(\"".$row->IDSteps."\", \"".$row->Steps."\")'>".lang('hapus')."</a>";
                    $text .= "</div>";
                }
