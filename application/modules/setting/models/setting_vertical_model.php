<?php
class Setting_Vertical_Model extends MY_Model
{
	/**
  ...
  protected $_temporary_with_deleted = FALSE;
	...
	protected $_department = 'BusinessDepartment';
	protected $_business_unit = 'BusinessUnit';
	...
	public function add(array $data){
		if ($this->check($data[$this->_name])){
			$this->set_error('vertical_creation_duplicate_name');
			return FALSE;
		}
		$this->set_message('vertical_creation_successful');
	...
	public function change(array $data,$id = null){
		...
		if (count($result) == 0)
		{
			$this->set_error('vertical_change_unsuccessful');
			return FALSE;
		}
		...
		if ($return)
		{
			$this->set_message('vertical_change_successful');
		}
		else
		{
			$this->set_error('vertical_change_unsuccessful');
	...
	public function delete(){
		...
		if ($delete){
			$this->set_message('vertical_delete_successful');
      $this->_aj['status'] = 'success';
			$this->_aj['message'] = $this->messages;
		}else {
			$this->set_errpr('vertical_delete_successful');
	...
	public function form(){
		$form_data = array('name','business_unit','department','hidden_id');
		foreach ($form_data as $val)
  			$data[$val] = $this->input->post($val);

		$memo = array(
			$this->_name => $data['name'],
			$this->_business_unit => $data['business_unit'],
    ...
    if ($record->num_rows() > 0) {
      foreach ($record->result_array() as $row) {
      	$serial = array(
      		'name' => $row[$this->_name],
      		'department' => $row[$this->_department],
      		'business_unit' => $row[$this->_business_unit],
