<?php
class Setting_Cost_Model extends MY_Model
{
	/**
	...
	protected $_table = 'tbrCostCenter';
	protected $primary_key = 'CostCenterID';
	protected $soft_delete = FALSE;
  protected $soft_delete_key = 'deleted';
  protected $_temporary_with_deleted = FALSE;

	protected $_name 		 =  'CostCenterName';
	...
	public function add(array $data){
		if ($this->check($data[$this->_name])){
			$this->set_error('costcenter_creation_duplicate_name');
			return FALSE;
		}
		$this->set_message('costcenter_creation_successful');
	...
	public function change(array $data,$id = null){
		...
		if (count($result) == 0)
		{
			$this->set_error('costcenter_change_unsuccessful');
			return FALSE;
		}
		...
		if ($return)
		{
			$this->set_message('costcenter_change_successful');
		}
		else
		{
			$this->set_error('costcenter_change_unsuccessful');
	...
	public function delete(){
		...
		if ($delete){
			$this->set_message('costcenter_delete_successful');
      $this->_aj['status'] = 'success';
			$this->_aj['message'] = $this->messages;
		}else {
			$this->set_errpr('costcenter_delete_successful');
    ...
    if ($record->num_rows() > 0) {
      foreach ($record->result_array() as $row) {
      	...
      	$action['edit'] = $this->auth->_allowed('Dashboard.CostCenter.Form.Edit') ? '<a class="btn btn-link" onclick="editG(\''.$row[$this->primary_key].'\', \''.$js.'\')">Edit</a>' : '';
      	$action['delete'] = $this->auth->_allowed('Dashboard.CostCenter.Delete') ? '<a class="btn btn-link" onclick="deleteG(\''.$row[$this->primary_key].'\')">Delete</a>' : '';
