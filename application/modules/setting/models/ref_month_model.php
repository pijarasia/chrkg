...
class Ref_Month_Model extends MY_Model
{
	/**
	...
	protected $_table = 'tbrMonth';
	protected $primary_key = 'MonthID';
    ...
    //
    //
	protected $_id 			 =  'MonthID';
	protected $_name 	     =  'MonthNameID';
	protected $_english      =  'MonthNameEN';
	...
	public function check($name = '')
	{
		...
		return $this->db->where('MonthNameID', $name)
						->count_all_results($this->_table) > 0;
	}

	public function delete($id)
	{
		$delete = $this->db->delete($this->_table, array('MonthID' => $id));
		if ($delete)
            return true;
        else
            return false;
	...
	{
		$result = $this->db->where('MonthID', $id)
			->count_all_results($this->_table) > 0;
		...
		if ($result == 0)
		{
    		...
    		$this->db->update($this->_table, $data, array('MonthID' => $id));
	...
	public function record()
	{
	    ...
	    if ($record->num_rows() > 0) {
            ...
            foreach ($record->result() as $row) {
                ...
                $bool_change = $this->auth->_allowed('Dashboard.Reference.Month.Change');
                $bool_delete = $this->auth->_allowed('Dashboard.Reference.Month.Delete');                
                ...
                if ($bool_change || $bool_delete)
                {
                    ...
                    if ($bool_change)
                        $text .= "<a class='btn btn-small btn-success' onclick='editM(\"".$row->MonthID."\", \"".$row->MonthNameID."\", \"".$row->MonthNameEN."\")'>".lang('edit')."</a>";
                    if ($bool_delete)
                        $text .= "<a class='btn btn-small btn-success' onclick='deleteM(\"".$row->MonthID."\", \"".$row->MonthNameID."\")'>".lang('hapus')."</a>";
                    $text .= "</div>";
                }
           
                $result['data'][] = array(	
    	          'id' => "{$row->MonthID}",
    	          'month' => "{$row->MonthNameID}",
    	          'english' => "{$row->MonthNameEN}",
    	          'action' => $text
    	        );
    	        $n++;
            }
