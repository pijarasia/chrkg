...
class Ref_Business_Model extends MY_Model
{
	/**
	...
	protected $_table = 'tbrBusinessArea';
	protected $primary_key = 'BusinessID';
	protected $soft_delete = FALSE;
  protected $soft_delete_key = 'deleted';
  protected $_temporary_with_deleted = FALSE;

	protected $_id 			 =  'BusinessID';
	protected $_name 		 =  'BusinessArea';
	...
	public function check($name = '')
	{
		...
		return $this->db->where('BusinessArea', $name)
						->count_all_results($this->_table) > 0;
	}

	public function delete($id)
	{
		$delete = $this->db->delete($this->_table, array('BusinessID' => $id));
		if ($delete)
            return true;
        else
            return false;
	...
	{
		$result = $this->db->where('BusinessID', $id)
			->count_all_results($this->_table) > 0;
		...
		$this->db->update($this->_table, $data, array('BusinessID' => $id));
	...
	public function record()
	{
	    ...
	    if ($record->num_rows() > 0) {
            ...
            foreach ($record->result() as $row) {
                $bool_change = $this->auth->_allowed('Dashboard.Reference.Business.Change');
                $bool_delete = $this->auth->_allowed('Dashboard.Reference.Business.Delete');
                ...
                if ($bool_change || $bool_delete)
                {
                    ...
                    if ($bool_change)
                        $text .= "<a class='btn btn-small btn-success' onclick='editB(\"".$row->BusinessID."\", \"".$row->BusinessArea."\")'>".lang('edit')."</a>";
                    if ($bool_delete)
                        $text .= "<a class='btn btn-small btn-success' onclick='deleteB(\"".$row->BusinessID."\", \"".$row->BusinessArea."\")'>".lang('hapus')."</a>";
                    $text .= "</div>";
                }

    	        $result['data'][] = array(
    	          'id' => "{$row->BusinessID}",
    	          'name' => "{$row->BusinessArea}",
    	          'action' => $text
    	        );
	           $n++;
            }
