...
class Ref_Employment_Model extends MY_Model
{
	/**
	...
	protected $_table = 'tbrEmploymentType';
	protected $primary_key = 'EmploymentTypeID';
    ...
    //
    //
	protected $_id 			 =  'EmploymentTypeID';
	protected $_name 		 =  'EmploymentType';
	...
	public function check($name = '')
	{
		...
		return $this->db->where('EmploymentType', $name)
						->count_all_results($this->_table) > 0;
	}

	public function delete($id)
	{
		$delete = $this->db->delete($this->_table, array('EmploymentTypeID' => $id));
		if ($delete)
            return true;
        else
            return false;
	...
	{
		$result = $this->db->where('EmploymentTypeID', $id)
			->count_all_results($this->_table) > 0;
		...
		$this->db->update($this->_table, $data, array('EmploymentTypeID' => $id));
	...
	public function record()
	{
	    ...
	    if ($record->num_rows() > 0) {
            ...
            foreach ($record->result() as $row) {
                $bool_change = $this->auth->_allowed('Dashboard.Reference.Employment.Change');
                $bool_delete = $this->auth->_allowed('Dashboard.Reference.Employment.Delete');                
                ...
                if ($bool_change || $bool_delete)
                {
                    ...
                    if ($bool_change)
                        $text .= "<a class='btn btn-small btn-success' onclick='editB(\"".$row->EmploymentTypeID."\", \"".$row->EmploymentType."\")'>".lang('edit')."</a>";
                    if ($bool_delete)
                        $text .= "<a class='btn btn-small btn-success' onclick='deleteB(\"".$row->EmploymentTypeID."\", \"".$row->EmploymentType."\")'>".lang('hapus')."</a>";
                    $text .= "</div>";
                }
