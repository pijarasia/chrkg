...
class Ref_Language_Model extends MY_Model
{
	/**
	...
	protected $_table = 'tbrAplLanguageLiteracy';
	protected $primary_key = 'aplLangCode';
    ...
    //
    //
	protected $_id 		   =  'aplLangCode';
	protected $_name 	   =  'aplLangEnglish';
	protected $_english    =  'aplLangBahasa';
	...
	public function check($name = '')
	{
		...
		return $this->db->where('aplLangEnglish', $name)
						->count_all_results($this->_table) > 0;
	}

	public function delete($id)
	{
        $delete = $this->db->delete($this->_table, array('aplLangCode' => $id));
	...
	{
		$result = $this->db->where('aplLangCode', $id)
			->count_all_results($this->_table) > 0;

		if ($result == 0)
		{
            $name = $data["aplLangEnglish"];
            $english = $data["aplLangBahasa"];
            $add = array('aplLangCode' => $id, 'aplLangEnglish' => $name, 'aplLangBahasa' => $english);
            return $this->add($level, $add);
		} else {	  
    		$this->db->update($this->_table, $data, array('aplLangCode' => $id));
	...
	public function record()
	{
	    ...
	    if ($record->num_rows() > 0) {
	      ...
	      foreach ($record->result() as $row) {
            $result['data'][] = array(	
	          'code' => "{$row->aplLangCode}",
	          'name' => "{$row->aplLangEnglish}",
	          'english' => "{$row->aplLangBahasa}",
	          'action' => "<div class='btn-group'>
						    <a class='btn btn-small btn-success' onclick='editL(\"".$row->aplLangCode."\", \"".$row->aplLangBahasa."\", \"".$row->aplLangEnglish."\")'>".lang('edit')."</a>
						    <a class='btn btn-small btn-success' onclick='deleteL(\"".$row->aplLangCode."\", \"".$row->aplLangEnglish."\")'>".lang('hapus')."</a>
						  </div>"
	        );
	        $n++;
	      }
