...
var Document = {
  ...
  url: '<?php echo base_url()."setting/permission";?>',
  search: function(n) {
    var $form = $("#sform");
...
var deleteG = function(id){
    ...
    $.ajax({
        url : "<?php echo base_url(); ?>setting/permission_delete",
        type : 'POST',
        data : form_data,
        success: function(d){
          $.pnotify({
...
var editG = function(id,js)
{
    ...
    $('#dialog').find('#status').val(obj.status);
    return false;
}
...
$(document).ready(function() {
  ...
  $("#save").click(function() {
    ...
    if(!$valid) {
      ...
      $.ajax({
        url: '<?php echo base_url()."setting/permission_form" ?>',
        type: 'POST',
        dataType: 'json',
