...
(function($) {
   ...
   * Attach functions.
   */
  Drupal.behaviors.hbossCzrSocial = {
    attach: function(context) {
      $('div.hboss-czr-social-facebook').each(function() {
        $(this).once(function() {
          var $this = $(this),
            idName = $this.attr('id').match(/hboss-czr-social-facebook-(.*)/)[1],
            thisWidth = $this.parent().width(),
            src = 'http://www.facebook.com/plugins/likebox.php?href=www.facebook.com/' + idName + '&width=' + thisWidth + '&height=590&colorscheme=light&show_faces=true&border_color&stream=true&header=true';
          $this.append(
            $('<iframe>').attr({
              'src': src,
              'scrolling': 'no',
              ...
              'width': thisWidth + 'px',
              // Where is 595px coming from? I don't know; it seems to fit, even
              // though in the URL above we ask for one 590px tall. Where does
              // 590px come from? I don't know. Why, if we don't add this CSS
              // parameter, is the iframe the size it is, even though there
              // doesn't seem to be any CSS which would dictate that size? I
              // don't know.
              'height': '595px'
            })
          );
